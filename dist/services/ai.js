 ...
 */
export const enhanceTask = async (partialTask) => {
    try {
...
`;
        ...
        const validEnergyLevels = ['XS', 'S', 'M', 'L', 'XL'];
        const validTimeNeeded = ['1min', '15min', '25min', 'more'];
...
export const startFocusSession = async (config) => {
    try {
...
DOSTĘPNE ZADANIA:
${availableTasks.length > 0 ? availableTasks.map(t => `- "${t.title}" | Priorytet: ${t.priority} | Potrzebna energia: ${t.energy_level} | Czas: ${t.time_needed} | Status zadania: ${t.status}`).join('\n') : 'Brak zadań do zrobienia'}
 ...
 */
export const executeQuickAction = async (action, context) => {
    try {
        ...
        console.log('[AI] Found', availableTasks.length, 'available todo tasks for next selection (after excluding skipped)');
...
${availableTasks.length > 0 ? availableTasks.map(t => `- "${t.title}" | Priorytet: ${t.priority} | Energia: ${t.energy_level} | Czas: ${t.time_needed}`).join('\n') : 'Brak więcej zadań'}
